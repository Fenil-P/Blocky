...
class MainState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Process the move done by current player.
        """
        self._current_player().process_event(event)

    def update(self) -> GameState:
        """Update the game by checking if maximum turns are completed and asking
        the players to make a move. Update the board if a move other than PASS
        was made by a player and return <self>. If maximum turn are completed,
        return game data including scores player id.
        """
    ...
    def render(self, renderer: Renderer) -> None:
        """Use <renderer> to display the board, player id, current turn and
...
class AnimateMoveState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Process the event from the operating system. Ignore the event."""
        return  # Ignore the event

    def update(self) -> GameState:
        """Update this GameState based on past events and animation for the
        most recent move. Return the next GameState that should be updated.
        This can be self.
        """
    ...
    def render(self, renderer: Renderer) -> None:
        """Use <renderer> to display the board, draw outline around the selected
        block, draw the image representing the move and update the status
        message.
        """
...
class GameOverState(GameState):
    ...
    def update(self) -> GameState:
        """Update GameState since game is over. Do nothing and return self."""
        # Nothing to change
        return self

    def render(self, renderer: Renderer) -> None:
        """Use <renderer> to display the winner, all player ids, and all final
