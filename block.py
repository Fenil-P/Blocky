...
class Block:
    ...
    def smash(self) -> bool:
        ...
        if self.smashable():
            ...
            for i in pos:
                rand = random.choice(COLOUR_LIST)
                self.children.append(Block(i, size, rand, level, depth))
            for child in self.children:
                i = random.random()
                if i < math.exp(-0.25 * self.level):
                    child.smash()
            return True
        else:
            return False
    ...
    def swap(self, direction: int) -> bool:
        ...
        if len(self.children) == 4:
            if direction == 1:
                ...
                top = [self.children[1], self.children[0]]
                bottom = [self.children[3], self.children[2]]
